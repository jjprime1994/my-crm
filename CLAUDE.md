...
A second nightly cron (`GET /api/cron/close-stale-leads`, 5am MYT, added 18 Aug 2026) auto-closes leads nobody has touched in 30+ days, in two independent sweeps — marks them `CLOSED_LOST`, logs a `LeadStatusHistory` row and a `LeadNote` explaining why. Same `CRON_SECRET`/SUPER_ADMIN auth pattern as check-routing. Logic lives in `src/lib/close-stale-leads.ts`.
  - **Unclaimed** (`closeStaleUnclaimedLeads`, `assignedToId: null`): keyed off `createdAt`, not `updatedAt` (fixed 19 Aug 2026) — an unclaimed lead's `updatedAt` can get bumped by non-engagement writes (state/routing corrections, backfills) and silently reset its staleness clock, which is what happened to ~100 leads via a bulk correction on 4 Aug 2026 and is why the cron found zero candidates despite leads visibly sitting unclaimed 30+ days in Available Leads (which displays `createdAt`).
  - **Claimed** (`closeStaleClaimedLeads`, added 19 Aug 2026, `assignedToId` set): keyed off `updatedAt`, same signal as the 48h "Stale" column on Team Breakdown, just acted on once it's been quiet a full month instead of only flagged. Ownership isn't touched, and status can be changed again afterward same as any lead — e.g. back to Won if the deal closes after all — this is Lost-by-default, not a lock.
