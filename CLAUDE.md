...
A second nightly cron (`GET /api/cron/close-stale-leads`, 5am MYT, added 18 Aug 2026) auto-closes leads that are still unclaimed (`assignedToId: null`) and were created 30+ days ago — marks them `CLOSED_LOST`, logs a `LeadStatusHistory` row and a `LeadNote` explaining why. Keyed off `createdAt`, not `updatedAt` (fixed 19 Aug 2026): an unclaimed lead's `updatedAt` can get bumped by non-engagement writes (state/routing corrections, backfills) and silently reset its staleness clock — this happened to ~100 leads via a bulk correction on 4 Aug 2026, which is why the cron found zero candidates despite leads visibly sitting unclaimed 30+ days in Available Leads (which displays `createdAt`). Same `CRON_SECRET`/SUPER_ADMIN auth pattern as check-routing. Logic lives in `src/lib/close-stale-leads.ts`. Deliberately scoped to the unclaimed pool only — a lead someone claimed and then went quiet on is a different problem (the "Stale" column) and isn't touched by this cron.
