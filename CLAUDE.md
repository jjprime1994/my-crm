...
A second nightly cron (`GET /api/cron/close-stale-leads`, 5am MYT, added 18 Aug 2026) auto-closes leads that are still unclaimed (`assignedToId: null`) and haven't had `updatedAt` change in 30+ days — marks them `CLOSED_LOST`, logs a `LeadStatusHistory` row and a `LeadNote` explaining why. Same `CRON_SECRET`/SUPER_ADMIN auth pattern as check-routing. Logic lives in `src/lib/close-stale-leads.ts`. Deliberately scoped to the unclaimed pool only — a lead someone claimed and then went quiet on is a different problem (the "Stale" column) and isn't touched by this cron.
