...
// Reaching out via WhatsApp or a call is a real contact attempt — logs the same
// "Contacted via" note the lead detail page does, which the API auto-advances a
// fresh (NEW) lead to CONTACTED on. Refresh so the status badge here catches up.
export default function ContactButtons({ leadId, phone, waLabel, waClassName, callClassName }: Props) {
  ...
  return (
    <>
      ...
      <a href={`tel:${phone}`} onClick={() => logContact("phone call")} className={callClassName}>
        {CALL_ICON}
        Call
      </a>
    </>
